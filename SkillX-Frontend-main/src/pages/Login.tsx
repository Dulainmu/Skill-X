...
const Login = () => {
  ...
  const handleMentorLogin = async () => {
    // Use the mentor credentials from the seed script
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.role === 'mentor') {
          ...
          alert('This account is not a mentor.');
        }
      } else {
        alert(data.message || 'Mentor login failed');
      }
    } catch (error) {
      alert('Mentor login failed. Please try again.');
    }
  };
